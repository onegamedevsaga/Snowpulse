...
std::shared_ptr<GraphicsOpenGLES> GraphicsOpenGLES::Create() {
    auto graphics = new GraphicsOpenGLES();
    return std::shared_ptr<GraphicsOpenGLES>(graphics);
}

GraphicsOpenGLES::GraphicsOpenGLES() {
}

void GraphicsOpenGLES::Initialize(Vector2Int resolution, Camera* camera, RenderQueueOpenGLES* renderQueue, Vector2Int screenSize) {
    std::cout << "Starting GLFW context, OpenGLES" << std::endl;
    /*if (!glfwInit()) {
...
void GraphicsOpenGLES::Shutdown() {
    ...
    textureSizes_.clear();
#ifdef SPDEBUG
    std::cout << "GraphicsOpenGLES shutdown." << std::endl;
#endif
}

GraphicsOpenGLES::~GraphicsOpenGLES() {
#ifdef SPDEBUG
    std::cout << "GraphicsOpenGLES destroyed." << std::endl;
#endif
}

Matrix4x4 GraphicsOpenGLES::InvertMatrixNatively(Matrix4x4 matrix) {
    glm::mat4 transform;
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
...
void GraphicsOpenGLES::LoadTexture(std::string filename, TextureFiltering filtering, PathType pathType) {
...
void GraphicsOpenGLES::UnloadTexture(std::string filename, TextureFiltering filtering) {
    auto fullFilename = GetTextureFullFilename(filename.c_str(), filtering);
    if (!textures_.count(fullFilename)) {
        return;
    }
...
Vector2 GraphicsOpenGLES::GetTextureSize(std::string filename, TextureFiltering filtering) {
    auto fullFilename = GetTextureFullFilename(filename.c_str(), filtering);
    if (!textures_.count(fullFilename)) {
...
int GraphicsOpenGLES::CreateRenderBatchGroup(int sortOrder) {
    int id = (int)renderBatchGroups_.size();
    auto group = std::make_shared<RenderBatchGroupOpenGLES>();
    group->sortOrder = sortOrder;
    renderBatchGroups_.push_back(group);
    return id;
}

void GraphicsOpenGLES::ClearRenderBatchGroups() {
    renderBatchGroups_.clear();
}

void GraphicsOpenGLES::SubmitRenderBatchGroup(int batchGroup) {
    if (batchGroup >= 0 && batchGroup < (int)renderBatchGroups_.size()) {
        renderQueue_->Push(renderBatchGroups_[batchGroup]);
    }
}

void GraphicsOpenGLES::DrawMesh(Vertex* vertices, unsigned int vertexCount, unsigned short* indices, unsigned int indexCount, std::string textureFullFilename, int sortOrder, BlendMode blendMode, bool isPremultiplied, Matrix4x4 transformMatrix, int batchGroup) {
    glm::mat4 transform;
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
    ...
    auto batch = std::make_shared<RenderBatchDataOpenGLES>();
...
void GraphicsOpenGLES::DrawSprite(Vector2 size, std::string textureFullFilename, Matrix4x4 transformMatrix, Color color, int sortOrder, BlendMode blendMode, bool isPremultiplied, Vector2 uvLowerLeft, Vector2 uvUpperRight, int batchGroup) {
